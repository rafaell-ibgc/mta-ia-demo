...
# Importa diretamente de 'mta' dentro de 'src/'
from mta import (
...
# ------------------------------
# Patch: gerar dados automaticamente e evitar tela em branco
# ------------------------------
...
# 1) Tentar ler upload, se houver
if uploaded is not None:
    try:
        with st.spinner('Lendo Excel enviado...'):
            xls = pd.ExcelFile(uploaded)
            required_sheets = {'touchpoints', 'conversions', 'channel_costs'}
            sheets_in_file = set(xls.sheet_names)
            missing = required_sheets - sheets_in_file
            if missing:
                st.error(f"As abas {missing} não foram encontradas no arquivo. "
                         "Use exatamente: 'touchpoints', 'conversions', 'channel_costs'. "
                         "Gerando dataset fictício como fallback...")
            else:
        ...
        st.error(f'Erro ao ler o Excel: {e}. Gerando dataset fictício como fallback...')

# 2) Se não há dados e o botão foi clicado, gerar conforme parâmetros
if (touchpoints is None or conversions is None or channel_costs is None) and btn_generate:
    with st.spinner('Gerando dataset fictício conforme configuração...'):
...
# 3) Se ainda não há dados (nem upload, nem botão), gerar automaticamente (default)
if touchpoints is None or conversions is None or channel_costs is None:
    with st.spinner('Preparando dataset fictício padrão...'):
        tp, cv, cc = generate_synthetic_dataset(n_users=5000, lookback_days=75)
        touchpoints, conversions, channel_costs = tp, cv, cc
        st.info('Geramos um dataset fictício padrão (5.000 usuários, janela 75 dias). '\
                'Você pode alterar na barra lateral e clicar em "Gerar Dataset Fictício".')

# ------------------------------
# Continuação: construção de paths e gráficos
# ------------------------------
